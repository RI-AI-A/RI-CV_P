...
from sqlalchemy import (
    ...
    Enum, Text, Index, TIMESTAMP, Boolean, UniqueConstraint, Date
)
...
class CustomerBranchMovement(Base):
    ...
    enter_time = Column(TIMESTAMP(timezone=True), primary_key=True, nullable=False)
    exit_time = Column(TIMESTAMP(timezone=True), nullable=True)
    action_type = Column(Enum(ActionType), nullable=False)
    event_hash = Column(String(64), nullable=True)  # Idempotency hash
    
    # Relationships
    customer = relationship("Customer", back_populates="movements")
    branch = relationship("Branch", back_populates="movements")
    
    # Indexes for time-based queries and idempotency
    __table_args__ = (
        Index("ix_customer_branch_movement_branch_id", "branch_id"),
        ...
        UniqueConstraint('event_hash', 'enter_time', name='uq_customer_branch_movement_event_hash'),
        {'postgresql_partition_by': 'RANGE (enter_time)'}
    )

class BranchDailyMetric(Base):
    """Aggregated daily metrics for branches."""
    __tablename__ = "branch_daily_metrics"
    ...
    __table_args__ = (
        Index("ix_branch_daily_metrics_branch_id", "branch_id"),
        Index("ix_branch_daily_metrics_date", "date"),
        UniqueConstraint('branch_id', 'date', name='uq_branch_daily_metrics_branch_date'),
    )
...
class Branch(Base):
    ...
    situations = relationship("Situation", back_populates="branch")


class Staff(Base):
    """Staff information (replacing/augmenting Employee)."""
    __tablename__ = "staff"
    ...
    shift_start = Column(TIMESTAMP(timezone=True), nullable=True)
    shift_end = Column(TIMESTAMP(timezone=True), nullable=True)
...
class Situation(Base):
    """Detected situations by the Situation Engine."""
    __tablename__ = "situations"
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    branch_id = Column(String(100), ForeignKey("branches.id"), nullable=False)
    situation_label = Column(String(100), nullable=False)
    severity = Column(Float, nullable=False)
    evidence = Column(JSONB, nullable=True)
    details = Column(Text, nullable=True)
    detected_at = Column(TIMESTAMP(timezone=True), default=datetime.utcnow, nullable=False)
    
    # Relationships
    branch = relationship("Branch", back_populates="situations")
    recommendations = relationship("Recommendation", back_populates="situation")
...
class Recommendation(Base):
    ...
    value_factor = Column(Float, default=0.0)
    created_at = Column(TIMESTAMP(timezone=True), default=datetime.utcnow, nullable=False)
    
    # Relationships
    situation = relationship("Situation", back_populates="recommendations")
